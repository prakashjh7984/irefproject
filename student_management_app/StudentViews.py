...
def states_change(request):
    ...
    return render(request,"student_template/edit_state_dropdown.html", {'states': states})
...
def payment_links(request):
    links=Links.objects.all()
    return render(request,"student_template/payment_links.html",{"links":links})

def class_links(request):
    links=Links.objects.all()
    return render(request,"student_template/class_links.html",{"links":links})
...
def view_marksheet(request,semester_id):
    # student=Students.objects.get(admin=request.user.id)
    student=Students.objects.get(admin=request.user.id)
    studentresult=StudentResult.objects.filter(student_id=student.id).filter(semester_id=semester_id)
